...
// 1) DASHBOARD
// ─────────────────────────────────────────────────────────────
function DashboardScreen({ onOpenCheckout, onOpenTxns }) {
  const [range, setRange] = useState('today');
  const HERO = {
    today: {
      label: 'SALES TODAY · ALL BRANCHES',
      value: 71575,
      sub: '64 sales · avg ₦1,118',
      spark: [4, 6, 9, 7, 12, 15, 18, 22, 26, 24, 31, 28],
      sparkLabels: ['8a','','10a','','12p','','2p','','4p','','6p',''],
      delta: '+12.4%',
      compare: 'vs yesterday',
    },
    week: {
      label: 'SALES THIS WEEK · ALL BRANCHES',
      value: 811200,
      sub: 'Avg ₦115,886 / day · 431 sales',
      spark: [68, 72, 51, 90, 84, 102, 95],
      sparkLabels: ['M','T','W','T','F','S','S'],
      delta: '+18.4%',
  ...
  return (
    <div style={{ paddingBottom: 120 }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              <div style={{ fontSize: 11, fontWeight: 700, opacity: 0.85, letterSpacing: '0.06em' }}>
                {HERO.label}
              </div>
              <div style={{
                ...
                {['today', 'week'].map(k => (
                  <button key={k} onClick={() => setRange(k)} style={{
                    ...
                    background: range === k ? 'rgba(255,255,255,0.95)' : 'transparent',
                    color: range === k ? '#1A1A22' : 'rgba(255,255,255,0.85)',
                    fontSize: 11, fontWeight: 700, letterSpacing: '0.02em',
                    cursor: 'pointer', fontFamily: 'inherit',
                    transition: 'all .15s ease',
                  }}>{k === 'today' ? 'Today' : 'Week'}</button>
                ))}
              </div>
            </div>
            <div style={{ marginTop: 8, fontFamily: '"JetBrains Mono", monospace', fontWeight: 700, fontSize: 38, letterSpacing: '-0.03em' }}>
              <span style={{ opacity: 0.7, marginRight: 2 }}>₦</span>{HERO.value.toLocaleString()}
            </div>
            <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 12, opacity: 0.8 }}>{HERO.compare}</span>
            </div>
            ...
            <div style={{ marginTop: 12, height: 42, color: 'rgba(255,255,255,0.85)' }}>
              <Sparkline key={range} data={HERO.spark} color="currentColor" height={42}/>
            </div>
            ...
            <div style={{
              ...
              {[
                ...
                { i: <Icons.refund size={18}/>, l: 'Refund', onClick: onOpenTxns },
                { i: <Icons.receipt size={18}/>, l: 'Receipts', onClick: onOpenTxns },
      ...
      {/* Recent activity */}
      <SectionHeader label="Recent activity" action="See all" onAction={onOpenTxns}/>
      <div style={{ padding: '0 20px' }}>
        <Card pad={6}>
          {RECENT_TX.map((t, i) => (
            <div key={t.id} style={{
