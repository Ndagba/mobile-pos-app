  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <NavBtn tab={TABS[0]} active={active === TABS[0].id} onClick={() => onChange(TABS[0].id)}/>
        <NavBtn tab={TABS[1]} active={active === TABS[1].id} onClick={() => onChange(TABS[1].id)}/>
        {/* center FAB */}
        ...
        <NavBtn tab={TABS[2]} active={active === TABS[2].id} onClick={() => onChange(TABS[2].id)}/>
        <NavBtn tab={TABS[3]} active={active === TABS[3].id} onClick={() => onChange(TABS[3].id)}/>
      </div>
    </div>
  );
};
...
const NavBtn = ({ tab, active, onClick }) => {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <span style={{ fontSize: 10, fontWeight: active ? 700 : 600, letterSpacing: '0.01em' }}>{tab.label}</span>
    </button>
  );
};
      ...
      <div style={themeStyle}>
        ...
        >
          ...
          <div style={{ width: '100%', height: '100%', overflowY: 'auto', overflowX: 'hidden', background: 'var(--bg)', color: 'var(--ink)' }}>
            {tab === 'dashboard' && <window.DashboardScreen onOpenCheckout={() => setCheckoutOpen(true)}/>}
            {tab === 'inventory' && <window.InventoryScreen/>}
            {tab === 'analytics' && <window.AnalyticsScreen/>}
            {tab === 'settings' && <window.SettingsScreen/>}
          </div>
